...
``` 
Access rstan object and use tidybayes to access a specific generated quantity
```{r}
rstan_obj <- estimates$estimates$fit
obs_reports <- rstan_obj |> tidybayes::spread_draws(obs_reports[t])
```

Now we will want to access our new transformed parameter, the wastewater
concentration.
```{r}
ww_conc <- rstan_obj |> tidybayes::spread_draws(ww_conc[t])
```
